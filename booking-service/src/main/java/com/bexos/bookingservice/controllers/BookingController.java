...
public class BookingController {
    ...
    public ResponseEntity<Accommodation> createAccommodationOffer(@RequestBody AccommodationRequest accommodationRequest) {
        Accommodation booking = bookingServiceImpl.createBookingOffer(bookingMapper.toAccommodation(accommodationRequest));
        return new ResponseEntity<>(booking, HttpStatus.CREATED);
    }

    @PostMapping("/hotel")
    public ResponseEntity<Hotel> createHotelOffer(@RequestBody HotelRequest hotelRequest) {
        Hotel booking = bookingServiceImpl.createBookingOffer(bookingMapper.toHotel(hotelRequest));
        return new ResponseEntity<>(booking, HttpStatus.CREATED);
    }

    @PostMapping("/flight")
    public ResponseEntity<Flight> createFlightOffer(@RequestBody FlightRequest flightRequest) {
        Flight booking = bookingServiceImpl.createBookingOffer(bookingMapper.toFlight(flightRequest));
        return new ResponseEntity<>(booking, HttpStatus.CREATED);
    }

    @PostMapping("/car-rental")
    public ResponseEntity<CarRental> createCarRentalOffer(@RequestBody CarRentalRequest carRentalRequest) {
